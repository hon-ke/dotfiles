...
class AuthAPI(BaseRemoteAPI):
    ...
    def refresh_api_key(self) -> Dict[str, Any]:
        """
        刷新 API Key
        ...
        endpoint = "/api/meta/refresh-api-key"
...
if __name__ == "__main__":
    # 初始化认证 API 客户端
    auth_api = AuthAPI(base_url="http://hon-ker",api_key = "")
    
    try:
        # 登录获取 API Key
        login_result = auth_api.login("astapi_blog", "5Lit5Zu912138#")
        print("登录结果:", login_result)
        api_key = login_result.get("api_key","")
        print("==",api_key,"==")
        auth_api.api_key = api_key
        # 验证 API Key
        verify_result = auth_api.verify_api_key()
